...
class Lidar:
    def __init__(self):
        ...
        self.cloud_sub = rospy.Subscriber(
            "/velodyne_points", PointCloud2, self.callback
        )
        self.cloud_pub = rospy.Publisher("/filtered_points", PointCloud2, queue_size=2)
        self.odom_sub = rospy.Subscriber(
            "/aft_mapped_adjusted", Odometry, self.odom_callback
        )
    ...
    def callback(self, ros_point_cloud):
        cloud_points = list(
            pc2.read_points(
                ros_point_cloud, skip_nans=True, field_names=("x", "y", "z")
            )
        )
        ...
        for x, y, z in cloud_points:
            ...
            if (
                self.min_distance_threshold <= distance <= self.max_distance_threshold
                and z > -1.07
                and z < 0
            ):
    ...
    def publish_filtered_points(self, filtered_points):
        ...
        if filtered_points[0]:
            points = list(
                zip(filtered_points[0], filtered_points[1], filtered_points[2])
            )
    ...
    def cluster_points(self):
        points_array = np.array(
            list(zip(self.points[0], self.points[1], self.points[2]))
        )

        db = DBSCAN(eps=0.4, min_samples=5).fit(points_array)
        labels = db.labels_
    ...
    def publish_boundaries(self, centroids, radii):
        ...
        for centroid, radius in zip(centroids, radii):
            point = Point32()
            point.x = (
                centroid[0] * np.cos(yaw - self.rotation)
                - centroid[1] * np.sin(yaw - self.rotation)
                + odom_x
            )
            point.y = (
                centroid[0] * np.sin(yaw - self.rotation)
                + centroid[1] * np.cos(yaw - self.rotation)
                + odom_y
            )
            point.x = centroid[0]
            point.y = centroid[1]
